...
def load_data():
    ...
    cosine_sim = pd.read_csv("cbf_cosine_sim.csv", index_col=0)
    return df_menu, cosine_sim

df_menu, cbf_cosine_sim = load_data()

# --- Fungsi Rekomendasi ---
def rekomendasi_cbf(menu_input, similarity_matrix, top_k=10):
    menu_input = menu_input.strip().upper()
    similarity_matrix.columns = similarity_matrix.columns.str.strip().str.upper()
    similarity_matrix.index = similarity_matrix.index.str.strip().str.upper()

    if menu_input not in similarity_matrix.columns:
        return []

    similar_items = similarity_matrix[menu_input].drop(index=menu_input)
    return similar_items.sort_values(ascending=False).head(top_k).index.tolist()
...
st.markdown(
    ...
    <div style="text-align: center; padding: 20px 0;">
        ...
        <p style="font-size: 18px; color: gray;">Content-Based Filtering (CBF)</p>
    </div>
    ...
    with col1:
        if f"rekomendasi_{iterasi}" not in st.session_state:
            if st.button("🎯 Tampilkan Rekomendasi", key=f"tampil_{iterasi}"):
                rekomendasi = rekomendasi_cbf(selected_menu, cbf_cosine_sim, top_k=10)
